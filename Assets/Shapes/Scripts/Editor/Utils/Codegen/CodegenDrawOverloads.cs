...
namespace Shapes {
	...
	public static class CodegenDrawOverloads {
		...
		const string DOC_REGPOL_ROUNDNESS = "A value from 0 to 1 setting how rounded the corners should be. 0 means sharp, 1 means fully rounded";
		const string DOC_REGPOL_THICKNESS = "The thickness of the border";
		...
		static List<string> GenerateAllOverloads() {
			...
			foreach( ( int fillType, string fillSuffix ) in fillTypes ) {
				foreach( ( bool hollow, string hollownessSuffix ) in new[] { ( false, "" ), ( true, "Hollow" ) } ) {
					foreach( bool usePositioning in positionedAndNonPositioned ) {
						...
						Param rpRoundness = new Param( $"float roundness // {DOC_REGPOL_ROUNDNESS}" );

						if( hollow ) {
							Param rpThickness = new Param( $"float thickness // {DOC_REGPOL_THICKNESS}" );
							g += new OrSelectorParams(
								null,
								new[] { rpRadius },
			...
			foreach( ( bool bordered, string suffixOvld ) in new[] { ( false, "" ), ( true, "Border" ) } ) {
				for( int mode = 0; mode < 4; mode++ ) {
					...
					g = new OverloadGenerator( "Rectangle" + suffixOvld, callTargets["Rectangle_Internal"], DOC_RECT ) { objectName = "rectangle" };
					if( bordered )
			...
			// Triangle
			g = new OverloadGenerator( "Triangle", callTargets["Triangle_Internal"], DOC_TRI );
